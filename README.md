...
欢迎提交可复现的 Issue 和 Pull Request。不要提交 `data`、`.browser`、模型、虚拟环境、缓存、密钥或包含个人内容的日志。

当前版本没有身份验证。绑定 `0.0.0.0` 时，同一局域网中的其他用户可以操作任务；不要直接暴露到互联网。`data` 和 `.browser` 可能包含个人数据或登录状态，分享前请脱敏。
